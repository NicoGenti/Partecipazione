 ...
 *
 * Images are distributed on a sphere using Fibonacci distribution and can be
 * rotated by drag (mouse / touch) with momentum, or auto-rotated. Clicking an
 * image opens a themed modal with the full picture.
 *
 * Adapted for the Partecipazione wedding invitation:
 * - Parchment / Hogwarts gold theming to match the app.
 * - Anti-download protections (no native drag, no context menu).
 * - Size driven by the parent so it stays responsive on small screens.
 */
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    ...
    // Collision detection to reduce overlaps
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="relative paper-surface rounded-sm max-w-md w-full overflow-hidden border-[3px] border-double border-[#d4af37] shadow-[0_20px_50px_rgba(0,0,0,0.6)]"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-9 h-9 bg-[#2c1d11] text-[#d6b772] rounded-full flex items-center justify-center hover:scale-110 transition-transform cursor-pointer border-2 border-[#d6b772] shadow-[0_4px_10px_rgba(0,0,0,0.5)]"
              aria-label="Chiudi"
            >
              <X size={16} />
            </button>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6 text-[#1a4a2e]">
              {selectedImage.title && (
                <h3 className="font-cinzel text-lg uppercase tracking-widest mb-2">{selectedImage.title}</h3>
              )}
              {selectedImage.description && (
                <p className="font-body text-sm text-[#1a4a2e]/70">{selectedImage.description}</p>
              )}
            </div>
          )}
        </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="rounded-full bg-[#1a4a2e]/5 animate-pulse flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="font-cinzel text-xs uppercase tracking-widest text-[#1a4a2e]/40">Caricamento…</div>
      </div>
    );
  }
